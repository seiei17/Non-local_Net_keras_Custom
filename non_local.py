...
def non_local(input, bottle_dim=None, compression=2,
    ...
    # compute f
    if mode == 'gaussian':
        xi = Reshape((-1, channel))(input)
        xj = Reshape((-1, channel))(input)
        f = Dot(axes=2)([xi, xj])
    ...
    if mode == 'embedded':
        ...
        theta = Reshape((-1, channel))(theta)
        phi = Conv2D(bottle_dim, (1, 1),
                     padding='same',
                     kernel_initializer=initial()
                     )(input)
        phi = Reshape((-1, channel))(phi)
        if compression > 1:
